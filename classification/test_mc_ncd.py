...
def main():
    ...
    # Verify W_A is zero
    assert torch.all(m_mc.W_A.weight == 0), "W_A must be zero-initialized"
    print(f"[OK] W_A.weight is all zero, shape={tuple(m_mc.W_A.weight.shape)}")
    ...
    if diff < 1e-4:
        print("[PASS] MC-NCD at init is numerically equivalent to VSSD baseline.")
    else:
        print("[FAIL] MC-NCD at init deviates from baseline. Check zero-init / a_bias derivation.")
