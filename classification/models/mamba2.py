...
# 对位置嵌入的相似度矩阵进行低秩分解，生成注意力机制的查询 / 键
def get_sine_svd(pos_embed, svd_dim=128, niter=2):
    pos_embed = pos_embed.flatten(0,2)
    pos_sim = pos_embed@pos_embed.T
    pos_softmax = F.softmax(pos_sim, dim=-1)
...
class VisionMamba3NC(nn.Module):
    """
    VisionMamba3NC-v2
    -----------------
    Non-causal visual mixer inspired by Mamba-3 SISO, but rewritten for vision.

    Key corrections vs. the earlier draft:
    1) angle branch follows angle_dt spirit: omega = pi * tanh(angle) * dt
    2) trap mainly modulates K / state-input strength, not V directly
    3) explicit diagonal term qk_dot * V is added to the output
    4) keep non-causal shared global memory instead of causal scan
    ...
    def __init__(
        ...
        bias=False,
        conv_bias=True,
        chunk_size=256,              # absorbed for interface compatibility
        use_mem_eff_path=False,      # absorbed
        layer_idx=None,              # absorbed
        device=None,
        dtype=None,
        linear_attn_duality=False,   # absorbed
        **kwargs,
    ):
        super().__init__()
        factory_kwargs = {"device": device, "dtype": dtype}
        ...
        self.multi_branch = kwargs.get("multi_branch", True)
        self.zact = kwargs.get("zact", False)
        self.use_cpe_local = kwargs.get("m3nc_use_pre_dwconv", True)
        self.local_kernel = kwargs.get("m3nc_local_kernel", 3)
        self.local_kernel = max(1, int(self.local_kernel))
        if self.local_kernel % 2 == 0:
            self.local_kernel += 1

        self.rope_base = kwargs.get("rope_base", 10000.0)
        rope_fraction = kwargs.get("rope_fraction", 0.5)
        rope_fraction = 0.5 if rope_fraction not in [0.5, 1.0] else rope_fraction

        self.split_tensor_size = int(self.d_state * rope_fraction)
        if self.split_tensor_size % 2 != 0:
            self.split_tensor_size -= 1
        self.split_tensor_size = max(2, min(self.split_tensor_size, self.d_state))
        ...
        # Lightweight visual local prior, similar in spirit to VSSD / vision mamba blocks
        conv_dim = self.d_inner + 2 * self.ngroups * self.d_state
        self.pre_dwconv = nn.Conv2d(
            in_channels=conv_dim,
            out_channels=conv_dim,
            groups=conv_dim,
            kernel_size=d_conv,
        ...
        nn.init.constant_(self.scale_dwconv.weight, 0.0)

        phase_channels = self.nheads * self.num_rope_angles
        self.phase_dwconv = nn.Conv2d(
            in_channels=phase_channels,
            out_channels=phase_channels,
            kernel_size=self.local_kernel,
            padding=self.local_kernel // 2,
            groups=phase_channels,
            bias=False,
            **factory_kwargs,
        )
        nn.init.constant_(self.phase_dwconv.weight, 0.0)

        self.act = nn.SiLU()

        # dt bias (same parameterization spirit as Mamba-2 / 3)
        dt = torch.exp(
        ...
        dt = torch.clamp(dt, min=dt_init_floor)
        inv_dt = dt + torch.log(-torch.expm1(-dt))
        self.dt_bias = nn.Parameter(inv_dt.to(dtype if dtype is not None else torch.float32))
        self.dt_bias._no_weight_decay = True

        # B / C bias: positive initialization aligns with Mamba-3 findings
        ...
        self.B_norm = RMSNormGated(self.d_state, eps=1e-5, norm_before_gate=False, **factory_kwargs)
        self.C_norm = RMSNormGated(self.d_state, eps=1e-5, norm_before_gate=False, **factory_kwargs)

        # Skip parameter
        self.use_dscale = kwargs.get("dscale", False)
        if self.use_dscale:
            self.D = Scale(dim=self.d_inner, init_value=1.0, trainable=True)
        else:
        ...
        self.qk_scale = self.d_state ** -0.5
        self.eps = 1e-6

        self.diag_scale = nn.Parameter(torch.tensor(-4.0))  # sigmoid(-4) ≈ 0.018

        # ===== MC-NCD: rank-r bottleneck for diagonal-A recovery =====
        # VisionMamba3NC's A = -softplus(dd_A) is (B, L, Hh) — data-dependent but
        # collapsed to one scalar per head, losing Mamba-2/3's diagonal expressiveness
        # along the d_state axis.  We recover it by a low-rank bottleneck:
        #     g_t      = SiLU(W_g(u))            in R^r
        #     a_delta  = W_A(g_t)                in R^{Hh x S}     (near-zero init)
        #     A_full   = A_coarse - softplus(a_delta)              in R^{B,L,Hh,S}
        # At init (W_A ~ 0) this is essentially the original A broadcast along S,
        # so the modified model contains VisionMamba3NC as a strict special case.
        self.use_mc_ncd = kwargs.get('use_mc_ncd', True)
        self.mc_rank = kwargs.get('mc_rank', 8)
        if self.use_mc_ncd:
            self.W_g = nn.Linear(self.d_model, self.mc_rank,
                                 bias=False, **factory_kwargs)
            self.W_A = nn.Linear(self.mc_rank, self.nheads * self.d_state,
                                 bias=False, **factory_kwargs)
            nn.init.trunc_normal_(self.W_g.weight, std=0.02)
            # Near-zero (not strict 0) so that DDP sees non-zero gradients on W_g
            # from step 1 onward.  std=1e-4 keeps step-0 deviation < 1e-3.
            nn.init.normal_(self.W_A.weight, mean=0.0, std=1e-4)
        # =============================================================
    ...
    # utility helpers
    # ------------------------------------------------------------------
    def _local_dwconv_scalar(self, x, H, W):
        B, L, Hh = x.shape
        x2d = x.permute(0, 2, 1).contiguous().view(B, Hh, H, W)

        raw_w = self.scale_dwconv.weight.view(self.nheads, 1, -1)
        weight = F.softmax(raw_w, dim=-1).view_as(self.scale_dwconv.weight)

        y2d = F.conv2d(
            x2d,
            weight=weight,
            bias=self.scale_dwconv.bias,
            stride=1,
            padding=self.local_kernel // 2,
            groups=self.nheads,
        )
        return y2d.view(B, Hh, L).permute(0, 2, 1).contiguous()

    def _local_dwconv_phase(self, x, H, W):
        """
        x: (B, L, Hh, A) -> (B, L, Hh, A)

        Learnable local phase aggregation.
        Uses softmax-normalized depthwise kernels.
        Zero-initialized kernels are exactly uniform local averaging.
        """
        B, L, Hh, A = x.shape
        assert Hh == self.nheads
        assert A == self.num_rope_angles

        x2d = x.permute(0, 2, 3, 1).contiguous().view(B, Hh * A, H, W)

        raw_w = self.phase_dwconv.weight.view(Hh * A, 1, -1)
        weight = F.softmax(raw_w, dim=-1).view_as(self.phase_dwconv.weight)
        ...
        y = y2d.view(B, Hh, A, L).permute(0, 3, 1, 2).contiguous()
        return y
    ...
    def _expand_groups_to_heads(self, x):
        # x: (B, L, G, N) -> (B, L, H, N)
        if self.ngroups == self.nheads:
            return x
        return x.repeat_interleave(self.heads_per_group, dim=2)

    def _local_avg_scalar(self, x, H, W):
        """
        x: (B, L, Hh) -> (B, L, Hh)
        symmetric 2D neighborhood average
        """
        B, L, Hh = x.shape
        x2d = x.permute(0, 2, 1).contiguous().view(B, Hh, H, W)
        y2d = F.avg_pool2d(x2d, kernel_size=self.local_kernel, stride=1, padding=self.local_kernel // 2)
        return y2d.view(B, Hh, L).permute(0, 2, 1).contiguous()

    def _local_avg_phase(self, x, H, W):
        """
        x: (B, L, Hh, A) -> (B, L, Hh, A)
        symmetric 2D neighborhood average over per-head phase field
        """
        B, L, Hh, A = x.shape
        x2d = x.permute(0, 2, 3, 1).contiguous().view(B, Hh * A, H, W)
        y2d = F.avg_pool2d(x2d, kernel_size=self.local_kernel, stride=1, padding=self.local_kernel // 2)
        return y2d.view(B, Hh, A, L).permute(0, 3, 1, 2).contiguous()

    def _apply_rotary_from_phase(self, x, phase):
        """
        x:     (B, L, Hh, N)
        phase: (B, L, Hh, A), where A >= rot_dim/2
        """
        B, L, Hh, N = x.shape
        rot_dim = min(self.split_tensor_size, N)
        if rot_dim < 2:
            return x
        if rot_dim % 2 != 0:
            rot_dim -= 1
        ...
        num_angles = rot_dim // 2
        phase = phase[..., :num_angles]

        x_rot = x[..., :rot_dim].reshape(B, L, Hh, num_angles, 2)
        xr = x_rot[..., 0]
        xi = x_rot[..., 1]

        cosv = torch.cos(phase)
        sinv = torch.sin(phase)

        yr = xr * cosv - xi * sinv
        yi = xi * cosv + xr * sinv

        out_rot = torch.stack([yr, yi], dim=-1).flatten(-2)
        out = torch.cat([out_rot, x[..., rot_dim:]], dim=-1)
        return out

    def _build_2d_phase(self, H, W, device, dtype):
        """
        Fixed 2D RoPE phase.
        ...
        if Ah > 0:
            freq_h = 1.0 / (
                    self.rope_base ** (
                    torch.arange(Ah, device=device, dtype=torch.float32) / max(1, Ah)
            )
            )
            phase_h = yy[:, None, :] * freq_h[None, None, :]  # (H, 1, Ah)
            phase_h = phase_h.expand(H, W, Ah)
            phases.append(phase_h)

        if Aw > 0:
            freq_w = 1.0 / (
                    self.rope_base ** (
                    torch.arange(Aw, device=device, dtype=torch.float32) / max(1, Aw)
            )
            )
            phase_w = xx[:, :, None] * freq_w[None, None, :]  # (1, W, Aw)
            phase_w = phase_w.expand(H, W, Aw)
            phases.append(phase_w)

        phase = torch.cat(phases, dim=-1)  # (H, W, A)
        phase = phase.reshape(1, H * W, 1, A).to(dtype=dtype)
        return phase

    # ------------------------------------------------------------------
    # main forward
    # ------------------------------------------------------------------
    def forward(self, u, H=None, W=None, relpos=None, last_kv=None):
        """
        u: (B, L, C)
        returns: (B, L, C)
        """
        ...
        if H is None or W is None:
            input_resolution = self.kwargs.get("input_resolution", None)
            assert input_resolution is not None, "H/W not provided and input_resolution is missing"
            H, W = input_resolution
        assert L == H * W, f"Expected L == H*W, got L={L}, H={H}, W={W}"

        proj = self.in_proj(u)

        if self.multi_branch:
            z, x, Bv, Cv, dd_dt, dd_A, trap, angles = torch.split(
                proj,
                [
                    self.d_inner,
                    self.d_inner,
            ...
            x, Bv, Cv, dd_dt, dd_A, trap, angles = torch.split(
                proj,
                [
                    self.d_inner,
                    self.ngroups * self.d_state,
        ...
        # --------------------------------------------------------------
        # 1) local visual preprocessing on [x, B, C]
        # --------------------------------------------------------------
        if self.use_cpe_local:
            xbc = torch.cat([x, Bv, Cv], dim=-1)
            xbc_2d = self._seq_to_2d(xbc, H, W)
            xbc_2d = self.act(self.pre_dwconv(xbc_2d))
            xbc = self._2d_to_seq(xbc_2d)
            x, Bv, Cv = torch.split(
                xbc,
        ...
        # --------------------------------------------------------------
        # 2) reshape to heads / groups
        # --------------------------------------------------------------
        V = rearrange(x, "b l (h p) -> b l h p", h=self.nheads, p=self.headdim)  # (B, L, Hh, P)

        Bv = rearrange(Bv, "b l (g n) -> b l g n", g=self.ngroups, n=self.d_state)
        Cv = rearrange(Cv, "b l (g n) -> b l g n", g=self.ngroups, n=self.d_state)

        Bv = self.B_norm(Bv)
        Cv = self.C_norm(Cv)

        Bv = self._expand_groups_to_heads(Bv)
        Cv = self._expand_groups_to_heads(Cv)

        # Q_pre / K_pre: bias added before rotation, consistent with official kernel order
        K_pre = Bv + self.B_bias.view(1, 1, self.nheads, self.d_state)
        Q_pre = Cv + self.C_bias.view(1, 1, self.nheads, self.d_state)

        # --------------------------------------------------------------
        # 3) dynamic dt / A / trap
        # --------------------------------------------------------------
        dt = F.softplus(dd_dt + self.dt_bias.view(1, 1, -1))                         # (B, L, Hh)
        A_coarse = -F.softplus(dd_A.to(torch.float32)).to(dt.dtype)                   # (B, L, Hh)
        A_coarse = torch.clamp(A_coarse, max=-self.A_floor)

        # ----- MC-NCD: expand A into per-(state-channel) diagonal -----
        # A_full: (B, L, Hh, S),  strictly <= A_coarse broadcast (always negative).
        if self.use_mc_ncd:
            g = F.silu(self.W_g(u))                                                   # (B, L, r)
            a_delta = self.W_A(g)                                                     # (B, L, Hh*S)
            a_delta = a_delta.view(B, L, self.nheads, self.d_state).to(dt.dtype)
            A_full = A_coarse.unsqueeze(-1) - F.softplus(a_delta)                     # (B, L, Hh, S)
            A_full = torch.clamp(A_full, max=-self.A_floor)
        else:
            A_full = A_coarse.unsqueeze(-1).expand(-1, -1, -1, self.d_state)          # (B, L, Hh, S)

        # Scalar surrogate (per-head) for any downstream code that wants (B, L, Hh).
        # Mean over d_state preserves the "average decay strength" semantics of A.
        A = A_full.mean(dim=-1)                                                       # (B, L, Hh)
        # --------------------------------------------------------------

        alpha = torch.exp(dt.unsqueeze(-1) * A_full)                                  # (B, L, Hh, S)
        lam = torch.sigmoid(trap)                                                    # (B, L, Hh)

        gamma = dt * lam                                                             # current endpoint
        beta = dt * (1.0 - lam)                # true previous-endpoint term in Mamba-3

        # non-causal symmetric counterpart of shifted_gamma
        beta_nc = self._local_dwconv_scalar(beta, H, W)                                   # (B, L, Hh)
        scale = gamma + beta_nc                                                       # K / write strength
        scale = scale / (scale.mean(dim=1, keepdim=True).detach() + 1e-4)
        scale = scale.clamp(0.0, 3.0)

        # global write weight: let A re-enter the model in a non-causal manner
        # m uses the scalar surrogate A (mean over d_state) to keep m's shape (B,L,Hh).
        log_m = torch.log(scale.clamp_min(self.eps)) + torch.log(F.softplus(-dt * A).clamp_min(self.eps))
        m = torch.softmax(log_m, dim=1)

        # --------------------------------------------------------------
        # 4) qk_dot diagonal term (before rotation)
        # official spirit: gamma * <Q_pre, K_pre>
        # --------------------------------------------------------------
        # qk_dot = gamma * (Q_pre * K_pre).sum(dim=-1) * self.qk_scale                 # (B, L, Hh)
        qk_self = (Q_pre * K_pre).sum(dim=-1) * self.qk_scale  # (B, L, Hh)

        # gamma 要和当前 scale 的量纲对齐
        gamma_norm = gamma / (scale.mean(dim=1, keepdim=True).detach() + 1e-4)
        gamma_norm = gamma_norm.clamp(0.0, 3.0)

        # 当前 memory self 权重是 m；如果你后续改成 K_rot * (m * scale)，这里也要同步
        diag_coeff = gamma_norm - m

        diag_gate = torch.sigmoid(self.diag_scale)
        diag = diag_gate * diag_coeff.unsqueeze(-1) * qk_self.unsqueeze(-1) * V

        # --------------------------------------------------------------
        # 5) angle_dt-consistent phase field, but non-causal
        # omega = pi * tanh(angle) * dt
        # then do symmetric local aggregation instead of prefix cumsum
        # --------------------------------------------------------------
        raw_angles = angles.unsqueeze(2).expand(-1, -1, self.nheads, -1)             # (B, L, Hh, A)
        omega = math.pi * torch.tanh(raw_angles) * dt.unsqueeze(-1)                  # (B, L, Hh, A)

        omega_local = self._local_dwconv_phase(omega, H, W)                             # (B, L, Hh, A)

        # weighted global phase center
        omega_center = (m.unsqueeze(-1) * omega).sum(dim=1, keepdim=True)            # (B, 1, Hh, A)

        phase_nc = omega + omega_local - omega_center                                # (B, L, Hh, A)

        phase_2d = self._build_2d_phase(
            H,
            W,
            device=u.device,
            dtype=phase_nc.dtype,
        )  # (1, L, 1, A)

        phase_nc = phase_2d + phase_nc

        # --------------------------------------------------------------
        # 6) rotate Q / K with the non-causal phase field
        # --------------------------------------------------------------
        Q_rot = self._apply_rotary_from_phase(Q_pre, phase_nc)
        K_rot = self._apply_rotary_from_phase(K_pre, phase_nc)

        # trap-consistent K scaling
        # K_scaled = K_rot * scale.unsqueeze(-1)                                       # (B, L, Hh, N)

        # --------------------------------------------------------------
        # 7) non-causal shared global memory
        # M = sum_t m_t * K_scaled_t^T V_t
        # --------------------------------------------------------------
        memory = torch.einsum(
            "blhn,blhp->bhnp",
            K_rot * m.unsqueeze(-1),
        ...
        # --------------------------------------------------------------
        # 8) add explicit diagonal term (D + qk_dot) * V
        # --------------------------------------------------------------
        if self.use_dscale:
            V_flat = rearrange(V, "b l h p -> b l (h p)")
            skip = rearrange(self.D(V_flat), "b l (h p) -> b l h p", h=self.nheads, p=self.headdim)
        else:
            skip = V * self.D.view(1, 1, self.nheads, 1)

        y = y_mem + skip + diag                                  # (B, L, Hh, P)

        # --------------------------------------------------------------
        # 9) flatten, norm, gate, out proj
        # --------------------------------------------------------------
        y = rearrange(y, "b l h p -> b l (h p)")
        y = self.norm(y)

        if self.multi_branch and z is not None:
            if self.zact:
                z = self.act(z)
            y = y * z
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
        ...
        if self.attn_type == 'standard':
            ...
            self.attn = Mamba2(d_model=dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ngroups=ssd_ngroups, chunk_size=ssd_chunk_size,
                                linear_attn_duality=linear_attn_duality, d_state=d_state, **kwargs)
        elif self.attn_type == 'mamba3':
            self.attn = VisionMamba3NC(
                d_model=dim,
...
class VMAMBA2(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.rmt_downsample = kwargs.get('rmt_downsample', False)
        self.rmt_patch_embed = kwargs.get('rmt_patch_embed', False)
        self.attn_types = kwargs.get('attn_types', ['mamba3', 'mamba3', 'mamba3', 'standard'])
        self.win_only = kwargs.get('win_only', [False]*len(depths))
        self.use_res_scale = kwargs.get('res_scale', [False]*len(depths))
        #param for SPT decay
        self.temp_ranges = kwargs.get('temp_ranges', [1,10,10,1000])
        ...
        #self.kv_scale = kwargs.get('kv_scale', [False]*len(depths))
        self.dscale = kwargs.get('dscale', [False]*len(depths))
        self.rope_fraction = kwargs.get("rope_fraction", [0.5]*len(depths))
        if not isinstance(d_state, (list, tuple)):
            d_state = [d_state] * len(depths)
        else:
            d_state = list(d_state)
            assert len(d_state) == len(depths), f"d_state length {len(d_state)} != num_layers {len(depths)}"
        if self.mlp_ratio is not list : self.mlp_ratio = [self.mlp_ratio]*len(depths)
        if kwargs.get('async_state', [None])[0] != None: #FIXME: pass state kwargs via dstate directly
            d_state = kwargs.get('async_state', [None])
        # if kwargs.get('async_mlp_ratio', [None])[0] != None: #FIXME: pass state kwargs via mlp_ratio directly
        ...
        # build layers
        print("DEBUG VMAMBA2 file:", __file__)
        print("DEBUG d_state before layer build:", d_state, type(d_state))
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            kwargs['attn_type'] = self.attn_types[i_layer]
            kwargs['win_only'] = self.win_only[i_layer]
            ...
            kwargs['dscale'] = self.dscale[i_layer]
            kwargs['preset_pos'] = self.svd_emds[i_layer]
            kwargs['rope_fraction'] = self.rope_fraction[i_layer]
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)),
                               depth=depths[i_layer],
...
if __name__ == '__main__':
    ...
    # 2. 初始化模型
    print("正在初始化模型...")
    model = VMAMBA2(img_size=224, depths=[2, 2, 8, 2], num_heads=[4, 4, 8, 16])
    model.to(device)
    ...
    print(f"总参数量 (百万): {total_params_m:.2f} M")
    print(f"总参数量 (十亿): {total_params_b:.3f} B")
    print(f"可训练参数量: {trainable_params:,}")

    # 估算模型权重显存占用（FP32）
    fp32_memory = total_params * 4 / 1024 / 1024
    print(f"\n模型权重显存估算 (FP32): {fp32_memory:.2f} MB")
    print(f"模型权重显存估算 (FP16/BF16): {fp32_memory / 2:.2f} MB")
